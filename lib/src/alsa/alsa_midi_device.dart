...
//
// Changes relative to upstream `midi` 0.1.1:
//  * Physical device removal (e.g. USB unplug) is now detected and surfaced via
//    [onDeviceDisconnected]. Upstream the receive isolate exited silently on
//    ENODEV/EBADFD without ever notifying listeners.
//  * [disconnect] is now idempotent and can be driven from both an explicit
//    disconnect and an unexpected isolate exit without firing twice or closing
//    ports more than once (which could crash on reconnect).
//  * [getDevices] now closes the per-card control handle it opens, so repeated
//    enumeration no longer leaks file descriptors.
...
Future<void> _rxIsolate(_RxIsolateArgs args) async {
  ...
  // Clean up
  calloc.free(buffer);
  // Signal that we've stopped. The main isolate distinguishes an expected stop
  // (it asked us to via [stopPort]) from an unexpected one (the read loop ended
  // on its own, e.g. the device was unplugged) by whether it is currently
...
class AlsaMidiDevice {
  ...
  Future<bool> connect() async {
    ...
    if ((status = alsa.snd_rawmidi_open(
      ...
      // Open failed: release the port cells we just allocated so a later
      // connect attempt starts clean and disconnect() has nothing to close.
      calloc.free(inPort!);
      calloc.free(outPort!);
      inPort = null;
      outPort = null;
    ...
    receivePort?.listen((data) {
      if (data == null) {
        final completer = _isolateStoppedCompleter;
        if (completer != null) {
          // Expected stop, initiated by disconnect().
          if (!completer.isCompleted) completer.complete();
        } else if (connected) {
          // The isolate exited on its own without us asking it to - the device
          // was most likely physically removed. Treat it as a disconnect.
  ...
  /// Called when the receive isolate exits unexpectedly (e.g. the device was
  /// physically removed). The isolate has already stopped, so we tear down
  /// without waiting for it.
  void _handleUnexpectedDisconnect() {
    disconnect(waitForIsolate: false);
  }

  /// Tears the device down and notifies [onDeviceDisconnected] listeners.
  ///
  /// Idempotent: this may be driven both by an explicit disconnect and by an
  /// unexpected isolate exit (unplug); it only cleans up and notifies once.
  ///
  /// When [waitForIsolate] is false the receive isolate is assumed to have
  /// already exited (so we don't wait for a stop acknowledgement that will
  /// never arrive).
  Future<void> disconnect({bool waitForIsolate = true}) async {
    if (!connected) return;
    connected = false;
  ...
  static List<AlsaMidiDevice> getDevices() {
    ...
    while (card.value >= 0) {
      ...
      // Close the control handle now that we've enumerated this card; the
      // device info we needed was read in the constructors above. Without this
      // each enumeration would leak a control file descriptor.
      alsa.snd_ctl_close(ctl.value);
